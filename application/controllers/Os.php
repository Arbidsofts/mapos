...
class Os extends MY_Controller
{
    ...
    public function adicionar()
    {
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if (is_numeric($id = $this->os_model->add('os', $data, true))) {
                ...
                $this->session->set_flashdata('success', 'O.S. adicionada com sucesso. Você pode adicionar produtos ou serviços a essa O.S. nas abas de Produtos e Serviços!');
                log_info('Adicionou uma O.S.');
    ...
    public function editar()
    {
        ...
        if (!$this->data['editavel']) {
            $this->session->set_flashdata('error', 'Esta O.S. já foi finalizada e seu status não pode ser alterado e nem suas informações atualizadas. Por favor abrir uma nova O.S.');
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if ($this->os_model->edit('os', $data, 'idOs', $this->input->post('idOs')) == true) {
                ...
                log_info('Alterou uma O.S. ID: ' . $this->input->post('idOs'));
    ...
    public function enviar_email()
    {
        ...
        // Verificar configuraçãoo de notificação
        $ValidarEmail = false;
        if ($this->data['configuration']['os_notification'] != 'nenhum') {
            $remetentes = [];
            switch ($this->data['configuration']['os_notification']) {
            ...
            if ($ValidarEmail) {
                if (empty($this->data['result']->email) || !filter_var($this->data['result']->email, FILTER_VALIDATE_EMAIL)) {
                    $this->session->set_flashdata('error', 'Por favor preencha o e-mail do cliente.');
            ...
            if ($enviouEmail) {
                $this->session->set_flashdata('success', 'O e-mail está sendo processado e será enviado em breve.');
    ...
    public function excluir()
    {
        ...
        if ($os == null) {
            ...
            if ($os == null) {
                $this->session->set_flashdata('error', 'Erro ao tentar excluir O.S.');
        ...
        if ($os->idCobranca != null) {
            if ($os->status == "canceled") {
                ...
                $this->session->set_flashdata('error', 'Existe uma cobrança associada a esta O.S., deve cancelar e/ou excluir a cobrança primeiro!');
        ...
        if ((int)$os->faturado === 1) {
            $this->os_model->delete('lancamentos', 'descricao', "Fatura de O.S. - #${id}");
        }

        log_info('Removeu uma O.S. ID: ' . $id);
        $this->session->set_flashdata('success', 'O.S. excluída com sucesso!');
        redirect(site_url('os/gerenciar/'));
    }
    ...
    public function adicionarProduto()
    {
        ...
        $os = $this->os_model->getById($id);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Erro ao tentar inserir produto na O.S.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }
        ...
        if ($this->os_model->add('produtos_os', $data) == true) {
            ...
            log_info('Adicionou produto a uma O.S. ID (O.S.): ' . $this->input->post('idOsProduto'));
    ...
    public function excluirProduto()
    {
        ...
        $os = $this->os_model->getById($idOs);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Erro ao tentar excluir produto na O.S.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }
        ...
        if ($this->os_model->delete('produtos_os', 'idProdutos_os', $id) == true) {
            ...
            log_info('Removeu produto de uma O.S. ID (O.S.): ' . $idOs);
    ...
    public function adicionarServico()
    {
        ...
        if ($this->os_model->add('servicos_os', $data) == true) {
            log_info('Adicionou serviço a uma O.S. ID (O.S.): ' . $this->input->post('idOsServico'));
    ...
    public function excluirServico()
    {
        ...
        if ($this->os_model->delete('servicos_os', 'idServicos_os', $ID) == true) {
            log_info('Removeu serviço de uma O.S. ID (O.S.): ' . $idOs);
            echo json_encode(['result' => true]);
        } else {
            echo json_encode(['result' => false]);
        }
    ...
    public function anexar()
    {
        ...
        $upload_conf = [
            ...
            'allowed_types' => 'jpg|png|gif|jpeg|JPG|PNG|GIF|JPEG|pdf|PDF|cdr|CDR|docx|DOCX|txt', // formatos permitidos para anexos de O.S.
            'max_size' => 0,
        ];
        ...
        if (count($error) > 0) {
            ...
            log_info('Adicionou anexo(s) a uma O.S. ID (O.S.): ' . $this->input->post('idOsServico'));
    ...
    public function excluirAnexo($id = null)
    {
        if ($id == null || !is_numeric($id)) {
            ...
            if ($this->os_model->delete('anexos', 'idAnexos', $id) == true) {
                log_info('Removeu anexo de uma O.S. ID (O.S.): ' . $idOs);
    ...
    public function faturar()
    {
        ...
        if ($this->form_validation->run('receita') == false) {
            ...
            if ($this->os_model->add('lancamentos', $data) == true) {
                ...
                log_info('Faturou uma O.S. ID: ' . $os);

                $this->session->set_flashdata('success', 'O.S. faturada com sucesso!');
                $json = ['result' => true];
                echo json_encode($json);
                die();
            } else {
                $this->session->set_flashdata('error', 'Ocorreu um erro ao tentar faturar O.S.');
                $json = ['result' => false];
                echo json_encode($json);
                die();
            }
    ...
    public function adicionarAnotacao()
    {
        ...
        if ($this->form_validation->run('anotacoes_os') == false) {
            ...
            if ($this->os_model->add('anotacoes_os', $data) == true) {
                log_info('Adicionou anotação a uma O.S. ID (O.S.): ' . $this->input->post('os_id'));
                echo json_encode(['result' => true]);
            } else {
                echo json_encode(['result' => false]);
            }
    ...
    public function excluirAnotacao()
    {
        ...
        if ($this->os_model->delete('anotacoes_os', 'idAnotacoes', $id) == true) {
            log_info('Removeu anotação de uma O.S. ID (O.S.): ' . $idOs);
            echo json_encode(['result' => true]);
        } else {
            echo json_encode(['result' => false]);
        }
