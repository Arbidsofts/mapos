...
if (!file_exists($settings_file)) {
    die("Arquivo de configura&ccedil;&atilde;o n&atilde;o encontrado!");
} else {
...
if (!empty($_POST)) {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        echo json_encode(["success" => false, "message" => "Por favor insira um e-mail v&aacute;lido."]);
        exit();
    }
    ...
    if (!is_file($settings['database_file'])) {
        echo json_encode(["success" => false, "message" => "O arquivo ../banco.sql n&atilde;o foi encontrado na pasta de instala&ccedil;&atilde;o!"]);
        exit();
    }
    ...
    if (!$is_installed) {
        echo json_encode(["success" => false, "message" => "Parece que este aplicativo j&aacute; est&aacute; instalado! Voc&ecirc; n&atilde;o pode reinstal&aacute;-lo novamente."]);
        exit();
    }
    ...
    echo json_encode(["success" => true, "message" => "Instala&ccedil;&atilde;o bem sucedida."]);
    exit();
}
